...
/* ── Handoff reasons — why a conversation needs a person ──────────────────── */
const HANDOFF_CFG: Record<string, { label: string; color: string; bg: string; border: string }> = {
  complaint:      { label: 'شكوى',            color: '#DC2626', bg: 'rgba(239,68,68,0.09)',  border: 'rgba(239,68,68,0.2)'  },
  human_request:  { label: 'طلب التحدث لموظف', color: '#DC2626', bg: 'rgba(239,68,68,0.09)',  border: 'rgba(239,68,68,0.2)'  },
  booking_manual: { label: 'تثبيت موعد',       color: '#D97706', bg: 'rgba(245,158,11,0.09)', border: 'rgba(245,158,11,0.2)' },
  owner_listing:  { label: 'عرض عقار للبيع',   color: '#7C3AED', bg: 'rgba(124,58,237,0.09)', border: 'rgba(124,58,237,0.2)' },
  general:        { label: 'متابعة عامة',      color: '#D97706', bg: 'rgba(245,158,11,0.09)', border: 'rgba(245,158,11,0.2)' },
};
// 'no_match' gets its own look everywhere — a sourcing gap, not a "reply to
// the customer" task, so it should never be mistaken for the others at a glance.
const NO_MATCH_CFG = { label: 'طلب غير متوفر', color: '#0891B2', bg: 'rgba(8,145,178,0.09)', border: 'rgba(8,145,178,0.2)' };
...
function ConvItem({ conv, isSelected, onClick }: { conv: any; isSelected: boolean; onClick: () => void }) {
  ...
  const needsHandoff = Boolean(conv.ai_handoff_requested);
  const isNoMatch = needsHandoff && conv.handoff_reason === 'no_match';
  const handoffCfg = needsHandoff ? (isNoMatch ? NO_MATCH_CFG : (HANDOFF_CFG[conv.handoff_reason] ?? HANDOFF_CFG['general'])) : null;

  return (
    <button onClick={onClick} className="w-full px-3 py-3 text-right transition-all border-b relative"
      style={{
      ...
      {/* A colored accent on the leading edge makes a flagged conversation
          catch the eye even while scanning past it quickly. */}
      {handoffCfg && (
        <span className="absolute top-0 bottom-0 right-0 w-1" style={{ background: handoffCfg.color }} />
      )}
        ...
        <div className="min-w-0 flex-1">
          ...
          {handoffCfg && (
            <span className="inline-flex items-center gap-1 mt-1.5 text-[10px] font-bold px-2 py-0.5 rounded-full"
              style={{ background: handoffCfg.bg, color: handoffCfg.color, border: `1px solid ${handoffCfg.border}` }}>
              {isNoMatch ? <MagnifyingGlassCircleIcon className="w-3 h-3" /> : <ExclamationTriangleIcon className="w-3 h-3" />}
              {handoffCfg.label}
            </span>
          )}
        </div>
...
/* ── Main Page ───────────────────────────────────────────────────────────── */
type Filter = 'all' | 'ai' | 'manual' | 'unread' | 'followup' | 'no_match';
...
export default function ConversationsPage() {
  ...
  const { data: convsRes } = useQuery({
    ...
    // limit: 100 (the API max) so the follow-up / no-match tabs below reflect
    // the real queue rather than just whatever fit in the default 20.
    queryFn:  () => conversationsApi.list({ ...(waInstance ? { instance: waInstance } : {}), limit: 100 }),
  ...
  const resolveHandoff = useMutation({
    mutationFn: () => conversationsApi.resolveHandoff(selectedConv.id),
    onSuccess: () => {
      toast.success('تم إغلاق المتابعة');
      setSelectedConv((p: any) => p ? { ...p, ai_handoff_requested: false, handoff_reason: null } : p);
      qc.invalidateQueries({ queryKey: ['conversations'] });
    },
  ...
  const filtered = allConvs.filter((c: any) => {
    ...
    const matchFilter = filter === 'all' || (filter === 'ai' && c.is_ai_enabled) ||
      ...
      (filter === 'followup' && needsFollowup(c)) || (filter === 'no_match' && isNoMatchReq(c));
  ...
  const counts = {
    ...
    followup: allConvs.filter(needsFollowup).length,
    no_match: allConvs.filter(isNoMatchReq).length,
  };
      ...
      {showList && (
        <div
          ...
          <div className="px-3 pt-3 pb-2 flex-shrink-0"
            ...
            {/* Two dedicated queues, kept visually apart from the routine
                tabs above: conversations waiting on a person, and requests
                the office simply doesn't have inventory for. */}
            {(counts.followup > 0 || counts.no_match > 0) && (
              <div className="flex gap-1.5 mt-1.5">
                {counts.followup > 0 && (
                  <button onClick={() => setFilter(filter === 'followup' ? 'all' : 'followup')}
                    ...
                    style={{
                      background: filter === 'followup' ? '#D97706' : 'rgba(245,158,11,0.09)',
                      color: filter === 'followup' ? '#fff' : '#D97706',
                      border: `1px solid ${filter === 'followup' ? '#D97706' : 'rgba(245,158,11,0.25)'}`,
                    }}>
                    <ExclamationTriangleIcon className="w-3.5 h-3.5" />
                    تحتاج متابعة بشرية
                    <span className="font-extrabold">{counts.followup}</span>
                  </button>
                )}
                {counts.no_match > 0 && (
                  <button onClick={() => setFilter(filter === 'no_match' ? 'all' : 'no_match')}
                    className="flex-1 flex items-center justify-center gap-1.5 px-2 py-1.5 rounded-lg text-[10px] font-bold transition-all"
                    style={{
                      background: filter === 'no_match' ? '#0891B2' : 'rgba(8,145,178,0.09)',
                      color: filter === 'no_match' ? '#fff' : '#0891B2',
                      border: `1px solid ${filter === 'no_match' ? '#0891B2' : 'rgba(8,145,178,0.25)'}`,
                    }}>
                    <MagnifyingGlassCircleIcon className="w-3.5 h-3.5" />
                    طلبات غير متوفرة
                    <span className="font-extrabold">{counts.no_match}</span>
                  </button>
                )}
              </div>
            )}
            ...
            {filtered.length === 0 ? (
              <div className="text-center py-12 px-4">
                ...
                <p className="text-sm font-medium" style={{ color: '#7A8FAA' }}>
                  {search ? 'لا نتائج' : filter === 'followup' ? 'لا توجد محادثات تحتاج متابعة'
                    : filter === 'no_match' ? 'لا توجد طلبات غير متوفرة حالياً' : 'لا توجد محادثات'}
                </p>
              </div>
      ...
      {showChat && (
        selectedConv ? (
          <div className="flex-1 flex flex-col min-w-0">
            ...
            {/* Handoff banner — why this conversation needs a person, and a
                one-click way to clear it once handled. */}
            {selectedConv.ai_handoff_requested && (() => {
              const isNoMatch = selectedConv.handoff_reason === 'no_match';
              const cfg = isNoMatch ? NO_MATCH_CFG : (HANDOFF_CFG[selectedConv.handoff_reason] ?? HANDOFF_CFG['general']);
              return (
                <div className="px-3 sm:px-4 py-2 flex items-center gap-2 flex-shrink-0 flex-wrap"
                  style={{ background: cfg.bg, borderBottom: `1px solid ${cfg.border}` }}>
                  {isNoMatch
                    ? <MagnifyingGlassCircleIcon className="w-4 h-4 flex-shrink-0" style={{ color: cfg.color }} />
                    : <ExclamationTriangleIcon className="w-4 h-4 flex-shrink-0" style={{ color: cfg.color }} />}
                  <span className="text-xs font-bold" style={{ color: cfg.color }}>
                    {isNoMatch ? 'طلب العميل غير متوفر لدينا حالياً' : `تحتاج متابعة بشرية — ${cfg.label}`}
                  </span>
                  <button onClick={() => resolveHandoff.mutate()} disabled={resolveHandoff.isPending}
                    className="mr-auto flex items-center gap-1 text-xs font-bold px-2.5 py-1 rounded-lg transition-all disabled:opacity-60"

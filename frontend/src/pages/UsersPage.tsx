...
export default function UsersPage() {
  ...
  const openEdit   = (u: any) => {
    setEditing(u);
    setForm({ full_name: u.full_name, full_name_ar: u.full_name_ar ?? '', email: u.email,
      password: '', role: u.role, whatsapp_instance: u.whatsapp_instance ?? '', is_active: u.is_active });
    setShowModal(true);
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">{users.length} موظف نشط</p>
        </div>
        {canManage && (
      ...
      <div className="flex gap-1 bg-gray-100 p-1 rounded-xl w-fit">
        <button onClick={() => setTab('active')}
          className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${tab === 'active' ? 'bg-white shadow text-gray-900' : 'text-gray-500 hover:text-gray-700'}`}>
          الموظفون النشطون
          <span className="mr-2 bg-blue-100 text-blue-700 text-xs px-1.5 py-0.5 rounded-full">{users.length}</span>
        </button>
        <button onClick={() => setTab('pending')}
          className={`px-4 py-2 rounded-lg text-sm font-medium transition-all flex items-center gap-2 ${tab === 'pending' ? 'bg-white shadow text-gray-900' : 'text-gray-500 hover:text-gray-700'}`}>
          طلبات الانضمام
          {pending.length > 0 && (
            <span className="bg-amber-400 text-white text-xs px-1.5 py-0.5 rounded-full animate-pulse">{pending.length}</span>
          )}
        </button>
      </div>
        ...
        <div className="space-y-3">
          {pending.length === 0 ? (
            <div className="bg-white rounded-xl border border-gray-100 py-16 text-center text-gray-400">
              ...
              <p className="text-sm">لا توجد طلبات انضمام معلقة</p>
            </div>
          ) : pending.map((u) => (
            <div key={u.id} className="bg-white rounded-xl border border-amber-100 shadow-sm p-4 flex items-center gap-4">
              {/* Avatar */}
              ...
              <div className="flex-1 min-w-0">
                <p className="font-semibold text-gray-900">{u.full_name_ar || u.full_name}</p>
                <p className="text-sm text-gray-500">{u.email}</p>
                <p className="text-xs text-gray-400 mt-0.5 flex items-center gap-1">
                  <ClockIcon className="w-3.5 h-3.5" />
                  طلب {format(new Date(u.created_at), 'dd MMM yyyy - HH:mm', { locale: ar })}
                </p>
              </div>
              ...
              {canManage && (
                <div className="flex items-center gap-3 flex-shrink-0">
                  <select
                    value={approveRole[u.id] ?? 'sales_agent'}
                  ...
                  <button
                    onClick={() => approveMut.mutate({ id: u.id, role: approveRole[u.id] ?? 'sales_agent' })}
                    disabled={approveMut.isPending}
                    className="flex items-center gap-1.5 bg-emerald-500 hover:bg-emerald-600 disabled:opacity-50 text-white px-3 py-1.5 rounded-lg text-xs font-medium transition">
                  ...
                  <button
                    ...
                    className="flex items-center gap-1.5 bg-red-50 hover:bg-red-100 disabled:opacity-50 text-red-500 px-3 py-1.5 rounded-lg text-xs font-medium transition">
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                <label className="block text-xs font-medium text-gray-600 mb-1">
                  كلمة المرور {editing && <span className="text-gray-400">(اتركها فارغة للإبقاء)</span>}
                </label>
